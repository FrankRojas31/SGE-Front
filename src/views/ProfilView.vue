...
const editedUser = ref({ name: '', email: authStore.auth.email || '', phone: '', address: '' });

const openModal = () => {
  editedUser.value = { name: '', email: authStore.auth.email || '', phone: '', address: '' };
  showModal.value = true;
};

const saveChanges = () => {
  // Update auth info
  authStore.auth.email = editedUser.value.email;
...
<template>
  <AppLayout>
    <div class="flex justify-center items-center  py-3">
      <div class="w-full max-w-4xl bg-white shadow-2xl rounded-3xl overflow-hidden transition-all duration-300">
        <div class="p-10">
          <Toast />
          <div class="flex flex-col items-center text-center space-y-4">
            <div
              ...
              <span class="text-5xl font-bold text-white">{{ authStore.auth.email?.charAt(0) || 'U' }}</span>
            </div>
            <h3 class="text-4xl font-bold text-gray-900 tracking-tight">{{ (authStore.auth.email || '').split('@')[0] ||
              'Usuario' }}</h3>
            <p class="text-xl text-gray-500">{{ authStore.auth.email }}</p>
          </div>
          ...
          <div class="mt-10 border-t border-gray-200 pt-8">
            ...
            <div class="space-y-4">
              <div class="flex items-center">
                ...
                <p class="text-lg text-gray-600">{{ editedUser.phone || 'No especificado' }}</p>
              </div>
              <div class="flex items-start">
                ...
                <p class="text-lg text-gray-600">{{ editedUser.address || 'No especificada' }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="mt-12 flex justify-center">
            <button @click="openModal"
              class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center space-x-2 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 focus:ring-offset-white">
